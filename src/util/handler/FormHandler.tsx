...
class FormHandler {
   ...
   * @param commentData - The comment data (name, email, message).
   */
  addComment = (
    postId: number,
    commentData: { name: string; email: string; message: string }
  ) => {
    if (
      ...
      toast.error("All fields are required to add a comment.");
      return;
    }


    // this.setForm((prevState: { posts: any[] }) => ({
    //   ...prevState,
    //   posts: prevState.posts?.map((post: { id: number; comments: any[] }) =>
    //     post.id === postId
    ...
    this.setForm((prevState: { comments: any }) => ({
      ...prevState,
  ...
  updateComment = (
    postId: number,
    commentId: number,
    updatedData: { name: string; email: string; message: string }
  ) => {
    this.setForm((prevState: { posts: any[] }) => ({
      ...prevState,
      posts: prevState.posts?.map((post: { id: number; comments: any[] }) =>
        post.id === postId
          ? {
              ...post,
              comments: post.comments.map((comment: { id: number }) =>
